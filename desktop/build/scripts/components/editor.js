...
function Editor(t, b) {
    ...
    // Parser
    this.cell_data = function (i, t, r, pattern, note) {
        ...
        if (marabu.cheatcode.is_active && i == marabu.selection.instrument && marabu.cheatcode.selection[r] || effect.cmd) {
            classes.bg = "b_inv f_inv";
        }
        else if (marabu.arp.is_active && marabu.arp.is_recording && i == marabu.selection.instrument && r >= marabu.selection.row && r <= marabu.selection.row + marabu.arp.memory.length) {
            classes.bg = "b_inv";
        }
        else if (marabu.arp.is_active && !marabu.arp.is_recording && i == marabu.selection.instrument && r == marabu.selection.row) {
            classes.bg = "b_inv f_inv";
        }
        if (marabu.cheatcode.is_active && i == marabu.selection.instrument && marabu.cheatcode.selection[r] || effect.cmd) {
            classes.fg = "f_inv";
            ...
            classes.fg = "b_inv f_inv";
        }
        else if (values.left || values.right) {
            classes.fg = "fm";
        }
        else if (pattern > 0 && r % 4 == 0) {
            classes.fg = "fm";
        }
        // Highlights
        if (r == marabu.selection.row && i == marabu.selection.instrument) {
            classes.fg = "selected";
        }
        // Compositor
        if (note) {
