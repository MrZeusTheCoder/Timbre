...
function CPlayer() {
    ...
    function createWaveHeader(waveWords, sampleRate, bitSize) {
        var headerLen = 44;
        var RIFF = new Uint8Array([82, 73, 70, 70]);
        var WAVE = new Uint8Array([87, 65, 86, 69]);
        //Format Chunk Identifier.
        var fmtI = new Uint8Array([102, 109, 116, 32]);
        //Length of the format data (16 for PCM). 
        var fmtL = 16;
        //PCM use Identifier.
        var PCMI = shiftTo8_4Array(0x000001);
        var channels = 2; //Stereo
        var sampleRateArray = shiftTo8_4Array(sampleRate);
        //I don't know what this is, tbf.
        var something = shiftTo8_4Array((sampleRate * bitSize * channels) / 8);
        var anotherSomething = new Uint8Array([(bitSize * channels) / 8, 0]);
        var bitDepth = new Uint8Array([bitSize, 0]);
        var dataHeader = new Uint8Array([100, 97, 116, 97]);
        var fullFileLength = headerLen + (waveWords * channels);
        var fileSize = shiftTo8_4Array(fullFileLength - 8);
        var dataSize = shiftTo8_4Array(fullFileLength - 44);
    ...
    this.createWave = function (bitDepth) {
