...
function CPlayerWorker() {
    ...
    function createNote(instr, n, rowLen) {
        ...
        var osc1 = mOscillators[signal_processor.osc_to_waveform(instr.i[0])[0]], o1vol = 255 - instr.i[1], o1xenv = instr.i[3], osc2 = mOscillators[signal_processor.osc_to_waveform(instr.i[0])[1]], o2vol = 255 - (255 - instr.i[1]), o2xenv = instr.i[3], noiseVol = instr.i[13], attack = instr.i[10] * instr.i[10] * 4, sustain = instr.i[11] * instr.i[11] * 4, release = instr.i[12] * instr.i[12] * 4, releaseInv = 1 / release, arp = 0, arpInterval = rowLen * Math.pow(2, 2 - 0);
    ...
    this.generate = function () {
        // Local variables
        var i, j, b, p, row, col, currentCol, n, cp, k, t, lfor, e, x, rsample, rowStartSample, f, da;
        for (currentCol = this.firstCol; currentCol <= this.lastCol; currentCol++) {
            // Put performance critical items in local variables
            var chnBuf = new Int32Array(this.numWords), mixBuf = this.mixBufWork, waveSamples = this.numSamples, waveWords = this.numWords, instr = this.song.songData[currentCol], rowLen = this.song.rowLen, patternLen = this.song.patternLen;
            // Clear effect state
            ...
            for (p = this.firstRow; p <= this.lastRow; ++p) {
                ...
                for (row = 0; row < patternLen; ++row) {
                    ...
                    var oscLFO = mOscillators[instr.i[15]], lfoAmt = instr.i[16] / 512, lfoFreq = Math.pow(2, instr.i[17] - 9) / rowLen, fxFilter = instr.i[19], fxFreq = instr.i[20] * 43.23529 * 3.141592 / 44100, q = 1 - instr.i[21] / 255, dlyAmt = instr.i[27] == 0 ? 0 : instr.i[26] / 255, dly = signal_processor.delay_conv(instr.i[27]) * rowLen;
