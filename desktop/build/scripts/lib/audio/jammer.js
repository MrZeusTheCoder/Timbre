...
*/
"use strict";
var SignalProcessor = require('./signal.js');
var Oscillator = require('./oscillator.js');
function CJammer() {
    var signal_processor = new SignalProcessor();
    ...
    function generateTimeSlice(leftBuf, rightBuf) {
        var numSamples = rightBuf.length;
        // Local variables
        var i, j, k, b, p, row, col, n, cp, t, lfor, e, x, rsample, rowStartSample, f, da;
        // Clear buffers
        ...
        for (i = 0; i < MAX_POLYPHONY; ++i) {
            ...
            if (note != undefined) {
                ...
                var osc1 = mOscillators[signal_processor.osc_to_waveform(note.instr[0])[0]], o1vol = 255 - note.instr[1], o1xenv = note.instr[3], osc2 = mOscillators[signal_processor.osc_to_waveform(note.instr[0])[1]], o2vol = 255 - (255 - note.instr[1]), o2xenv = note.instr[3], noiseVol = note.instr[13], attack = Math.round(note.instr[10] * note.instr[10] * 4 * mRateScale), sustain = Math.round(note.instr[11] * note.instr[11] * 4 * mRateScale), release = Math.round(note.instr[12] * note.instr[12] * 4 * mRateScale), releaseInv = 1 / release, arpInterval = mRowLen * Math.pow(2, 2 - 0);
                ...
                for (j = note.env, k = 0; k < samplesLeft; j++, k++) {
                    ...
                    else if (envelope_shape == 2) {
                        t *= e * e * e;
                    }
                    else if (envelope_shape == 3) {
                        t *= e * e * e * e;
                    }
                    o1t += t;
        ...
        // And the effects...
        var pos = mFXState.pos, low = mFXState.low, band = mFXState.band, filterActive = mFXState.filterActive, dlyPos = mFXState.dlyPos;
        var lsample, high, dlyRead, dlyMask = MAX_DELAY - 1;
        // Put performance critical instrument properties in local variables
        var oscLFO = mOscillators[mInstr[15]], lfoAmt = mInstr[16] / 512, lfoFreq = Math.pow(2, mInstr[17] - 9) / mRowLen, fxFilter = mInstr[19], fxFreq = mInstr[20] * 43.23529 * 3.141592 / mSampleRate, q = 1 - mInstr[21] / 255, dlyAmt = mInstr[27] == 0 ? 0 : mInstr[26] / 255, dly = (signal_processor.delay_conv(mInstr[27]) * mRowLen) >> 1;
