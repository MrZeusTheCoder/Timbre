...
var Controller = /** @class */ (function () {
    ...
    Controller.prototype.add = function (mode, cat, label, fn, accelerator) {
        if (!this.menu[mode]) {
            this.menu[mode] = {};
        }
        ...
        this.menu[mode][cat][label] = {
            fn: fn, accelerator: accelerator
        };
        console.log(mode + "/" + cat + "/" + label + " <" + accelerator + ">");
    };
    Controller.prototype.add_role = function (mode, cat, label) {
        if (!this.menu[mode]) {
            this.menu[mode] = {};
        }
        if (!this.menu[mode][cat]) {
    ...
    Controller.prototype.set = function (mode) {
        if (mode === void 0) { mode = "default"; }
        this.mode = mode;
        this.commit();
    };
    Controller.prototype.commit = function () {
        electron_1.Menu.setApplicationMenu(electron_1.Menu.buildFromTemplate(this.format()));
    };
    Controller.prototype.format = function () {
        var f = [];
        var m = this.menu[this.mode];
        for (var cat in m) {
            var submenu = [];
            for (var name in m[cat]) {
